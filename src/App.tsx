...
const liftIcons: Record<LiftId, typeof Barbell> = {
  squat: PersonSimpleTaiChi,
  bench: PersonArmsSpread,
  deadlift: PersonSimpleHike,
}
...
function SessionRow({ session, onStart }: { session: TrainingSession; onStart: () => void }) {
  ...
  const LiftIcon = liftIcons[session.lift]
  return (
    <article className={`session-row ${session.completedAt ? 'completed' : ''}`}>
      <div className={`lift-symbol lift-symbol-${session.lift}`} aria-hidden="true"><LiftIcon weight="bold" /></div>
